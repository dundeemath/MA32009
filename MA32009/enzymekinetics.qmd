...
In Figure \ref{BrussNondimFig} we use Maple to numerically solve @eq-brussnondim for the two cases and plot the solutions in the phase plane. Note that when the steady state is unstable, the numerical solution indicates that the system has limit cycle solutions. A valid question to ask is whether one can prove that this is the case.
